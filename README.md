...
| 层 | 技术 |
|----|------|
| 前端 | Vue 3 + Vite + vue-router + qrcode + jsPDF |
| 后端 | FastAPI + uvicorn |
| 包管理 | pnpm (前端) / uv (后端) |
...
共享电脑无需输入用户名。点击“打印明信片”会打开单页 PDF 预览（宽度固定 148mm，高度按底图比例推导，当前底图 2048 × 1360 下为 148mm × 98.3mm）；PDF 成功打开后，网页会清空本次录音和图片并回到初始状态。服务器文件继续保留，确保明信片二维码长期可用。
