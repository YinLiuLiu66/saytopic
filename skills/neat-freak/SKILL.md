...
> **Agent 记忆系统的具体位置因平台而异**（Claude Code 在 `~/.claude/projects/<...>/memory/`，Codex 用 `AGENTS.md`，OpenCode 用 `.opencode/`，OpenClaw 用 `~/.openclaw/`）。如果当前 agent 没有独立的记忆系统，直接跳过这一层，把功夫全花在 docs 和项目根 markdown 上。
...
1. 列出 agent 的记忆文件（如有）：
   ...
   - Codex / OpenCode / 其他：找该 agent 的等价位置
2. 对本次对话涉及的**每一个项目**：
